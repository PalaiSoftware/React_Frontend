...
const Purchase = () => {
  ...
  /* ------------------------------------------------------------------ */
  /* AUTOCOMPLETE SETUP - MOBILE BOTTOM MARGIN FIX */
  /* ------------------------------------------------------------------ */
  const setupAutocomplete = (rowId, inputRef, isEdit = false) => {
    ...
    const updatePosition = () => {
      ...
      // Add 200px clearance on mobile for keyboard
      ...
      suggestionsContainer.style.marginBottom = `${bottomMargin}px`;
                    ...
                    <React.Fragment key={tid}>
                      <tr key={`main-row-${tid}`}>
                        ...
                        <td className="px-4 py-3 text-sm flex gap-2">
                          ...
                          <button onClick={() => openEdit(p.transaction_id)} className="bg-green-600 text-white hover:bg-red-700 px-3 py-1.5 rounded-md">Edit</button>
                ...
                return (
                  <div key={`mobile-card-${tid}`} className="border rounded-lg p-4 bg-white shadow-sm">
                    ...
                    <div className="mt-3 flex gap-2">
                      <button onClick={() => toggleDetails(tid)} className="bg-cyan-600 text-white hover:bg-cyan-700 px-3 py-1.5 rounded-md text-sm">
                        {isOpen ? 'Hide Details' : 'View Details'}
                      </button>
                      <button onClick={() => openEdit(p.transaction_id)} className="bg-green-600 text-white hover:bg-red-700 px-3 py-1.5 rounded-md text-sm">Edit</button>
                      <button onClick={() => deletePurchase(p.transaction_id)} className="bg-red-600 text-white hover:bg-red-700 px-3 py-1.5 rounded-md text-sm ">Delete</button>
                    </div>
                    {isOpen && details && (
          ...
          <div className="bg-white rounded-lg max-w-6xl w-full max-h-screen overflow-y-auto">
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="border rounded-md overflow-hidden">
                ...
                {/* SUGGESTIONS - ADD (No max-h-48, Mobile Margin via JS) */}
                {purchaseItems.map(item => (
                  <div
                    key={`add-suggestions-portal-${item.id}`}
                    id={`add-suggestions-${item.id}`}
                    className="fixed bg-white border shadow-lg rounded-md mt-1 w-64 z-50 hidden overflow-y-auto"
              ...
              <div className="flex gap-2 justify-end">
                <button onClick={savePurchase} className="bg-cyan-600 text-white p-2 rounded-md hover:bg-cyan-700">
                  Save Purchase
                </button>
                <button onClick={() => { setShowAddPurchase(false); setPurchaseItems([]); }} className="bg-gray-200 p-2 rounded-md hover:bg-gray-300">
                  Cancel
                </button>
              </div>
            </div>
