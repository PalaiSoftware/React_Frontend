...
export default function ProductInfo() {
  ...
  // NEW: highlightedProducts holds IDs of products that should show red/yellow for 3 minutes
  ...
  const fetchProductInfo = async () => {
    ...
    try {
      ...
      // ---- Highlight logic: mark low-stock items and clear after 3 minutes ----
      ...
      const lowStockIds = new Set(
        list
          .filter((p) => {
            const v = parseInt(p.current_stock) || 0;
            return v === 0 || (v > 0 && v <= 10);
          })
          .map((p) => String(p.id))
      );
      ...
      if (lowStockIds.size > 0) {
        setHighlightedProducts(lowStockIds);
  ...
  /* ---------- Colour helper ---------- */
  // Now accepts id and stock; shows red/yellow only if id is in highlightedProducts
  const getStockColor = (id, stock) => {
    const v = parseInt(stock) || 0;
    if (highlightedProducts && highlightedProducts.has(String(id))) {
              ...
              <tbody className="divide-y divide-gray-200 bg-white">
                {paginated.length === 0 ? (
                  ...
                  paginated.map((p, i) => {
                    ...
                    // update: pass id to color helper
                    const colorCls = getStockColor(p.id, p.current_stock);
