...
// ---------------------------------------------------------------------
// Current time: November 13, 2025 10:30 PM IST
    ...
    try {
      ...
      setTimeout(() => {
        items.forEach(itm => {
          ...
          if (inp) {
            inp.value = itm.product_name || ''; // Show product name in input
                  ...
                  <tbody>
                    {editSaleItems.map(item => (
                      <tr key={`edit-sale-item-${item.id}`} data-row-id={item.id}>
                        ...
                        <td className="p-1"><input type="number" step="0.01" defaultValue={item.quantity} onChange={e => setEditSaleItems(prev => prev.map(i => i.id === item.id ? { ...i, quantity: parseFloat(e.target.value) || 0, total: calcRowTotal({ ...i, quantity: parseFloat(e.target.value) || 0 }) } : i))} className="w-16 p-1 border rounded" /></td>
                        ...
                        <td className="p-1"><input type="number" className="sale-discount w-16 p-1 border rounded" defaultValue={item.discount} onChange={e => setEditSaleItems(prev => prev.map(i => i.id === item.id ? { ...i, discount: parseFloat(e.target.value) || 0, total: calcRowTotal({ ...i, discount: parseFloat(e.target.value) || 0 }) } : i))} /></td>
                        <td className="p-1"><input type="number" className="sale-gst w-16 p-1 border rounded" defaultValue={item.gst} onChange={e => setEditSaleItems(prev => prev.map(i => i.id === item.id ? { ...i, gst: parseFloat(e.target.value) || 0, total: calcRowTotal({ ...i, gst: parseFloat(e.target.value) || 0 }) } : i))} /></td>
                        <td className="p-1"><input type="number" step="0.01" className="sale-cost w-full p-1 border rounded" defaultValue={item.cost} onChange={e => setEditSaleItems(prev => prev.map(i => i.id === item.id ? { ...i, cost: parseFloat(e.target.value) || 0, total: calcRowTotal({ ...i, cost: parseFloat(e.target.value) || 0 }) } : i))} /></td>
