...
export default function SalesDashboard() {
  ...
  // -----------------------------------------------------------------
  // Add Sale – send name with date
  // -----------------------------------------------------------------
  const handleAddSale = async e => {
    e.preventDefault();
    const form = e.target;

    // Build name with date
    const rawName = form.nameWithDate?.value?.trim() || '';
    const date = form.saleDateTime.value;
    const formattedDate = date
      ? new Date(date).toISOString().slice(0, 19).replace('T', ' ')
      : '';
    const nameWithDate = rawName ? `${rawName} ${date.slice(0, 16).replace('T', ' ')}` : formattedDate;
  ...
  // -----------------------------------------------------------------
  // saveEditedSale – send name with date
  // -----------------------------------------------------------------
  const saveEditedSale = async () => {
    ...
    const date = editForm.dateTime;
    const formattedDate = date ? `${date.replace('T', ' ')}:00` : new Date().toISOString().slice(0, 19).replace('T', ' ');
    const nameWithDate = rawName ? `${rawName} ${date.slice(0, 16).replace('T', ' ')}` : formattedDate;
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-screen overflow-y-auto">
            <form onSubmit={handleAddSale}>
              ...
              <div className="p-4 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <input name="nameWithDate" placeholder="Bill Name (date will be appended)" className="p-2 border rounded" required />
