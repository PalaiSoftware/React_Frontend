...
const paymentModeMap = {
  ...
  bank_transfer:Analyse 5, phonepe: 6
};
...
export default function SalesDashboard() {
  ...
  const [editForm, setEditForm] = useState({
    billName: '', dateTime: '', paymentMode: 'cash',
  ...
  // -----------------------------------------------------------------
  // Add Sale – FIXED: send discount instead of dis
  // -----------------------------------------------------------------
  const handleAddSale = async e => {
    ...
    const payload = {
      ...
      products: itemsWithTotal.map(i => ({
        ...
        discount: i.discount,   // ← CHANGED FROM 'dis'
  ...
  // -----------------------------------------------------------------
  // EDIT SALE – FIXED: send discount instead of dis
  // -----------------------------------------------------------------
  ...
  // -----------------------------------------------------------------
  // saveEditedSale – FULLY FIXED: sends discount
  // -----------------------------------------------------------------
  const saveEditedSale = async () => {
    ...
    const payload = {
      bill_name: editForm.billName?.trim() || '',
      ...
      updated_at: editForm.dateTime
        ? `${editForm.dateTime.replace('T', ' ')}:00`
            ...
            <div className="p-4 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <input
                  ...
                  onChange={e => setEditForm(p => ({ ...p, billName: e.target.value }))}
